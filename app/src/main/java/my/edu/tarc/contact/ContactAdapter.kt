...
class ContactAdapter : RecyclerView.Adapter<ContactAdapter.ViewHolder>() {
    ...
    //each node hold a contact record
    private var contactList = emptyList<Contact>()

    //View holder is to hold one single record
    //each record has 2 content 'textviewname and textview contact"
    //come frm onCreate ViewHolder
    ...
    //for adapter to receive a list of record to be shown
    internal fun setContact(contact: List<Contact>){
        this.contactList = contact
        //tell system there is a change.
